...
export default class BoxLayout {
  ...
  applyTo(target, parentBounds) {
    ...
    target.width = hasLeft && hasRight
      ? parentBounds.width - left - right
      : this.width.resolve(parentBounds.width, target.width)
    target.height = hasTop && hasBottom
      ? parentBounds.height - top - bottom
      : this.height.resolve(parentBounds.height, target.height)
